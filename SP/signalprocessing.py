...
A4_FREQ        = 440.0
SILENCE_THRESH = 0.02      # tuned for 110 dB SPL mic; bump by 0.005 if sustain sticks
...
FLUX_HISTORY   = 22   # ~0.25 s of history for adaptive onset threshold
FLUX_MULT      = 1.2  # slightly lower — onset is now sole trigger so must be reliable

# ── Octave correction ────────────────────────────────────────────────
# Physics: a vibrating string at fundamental f produces energy at
# 1f, 2f, 3f, 4f, ... but NEVER at f/2, f/3, ... So if the spectrum
# shows significant energy one or more octaves below the HPS peak,
# HPS must have locked onto a harmonic and the true fundamental is
# lower. We iterate down by octaves while each sub-octave bin still
# dominates its current peak by `OCTAVE_DOWN_THRESH`.
OCTAVE_DOWN_THRESH = 0.5    # sub-octave bin >= this × peak bin → go down
...
class PitchDetector:
    ...
    def _detect_pitch(self, mag_full: np.ndarray) -> float | None:
        ...
        # ── Iterated octave correction ─────────────────────────────────
        # HPS can lock onto a harmonic (typically the 2nd → one octave too
        # high, but occasionally the 4th → two octaves too high for
        # bright/percussive sounds). Because real strings can't produce
        # subharmonics, the presence of energy at f/2 always means HPS
        # missed the actual fundamental. Loop the check so a 2-octave
        # error gets corrected fully rather than stopping after one step.
        peak_bin = int(round(refined_bin))
        for _ in range(OCTAVE_DOWN_MAX):
            half_bin = peak_bin // 2
            if half_bin < 1:
                break
            if mag_full[half_bin] <= OCTAVE_DOWN_THRESH * mag_full[peak_bin]:
                break
            candidate_freq = freq_hz / 2
            candidate_midi = freq_to_midi(candidate_freq)
            if not (21 <= candidate_midi <= 108):
                break
            peak_bin = half_bin
            freq_hz = candidate_freq
        # ──────────────────────────────────────────────────────────────

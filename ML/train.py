"""
Train JazzLSTM on the 16th-note grid produced by process_to_grid.py.
...
Saves:
    ML/checkpoints/jazz_lstm.pt   (model state + meta needed to rebuild it)

CLI (all optional):
    python ML/train.py --epochs 30 --steps_per_epoch 200 --batch_size 64 --seq_len 256
"""
...
from model import JazzLSTM, build_chord_tables, UNK_ROOT_ID


DATA_DIR = os.path.join("ML", "data", "processed")
CKPT_DIR = os.path.join("ML", "checkpoints")
CKPT_PATH = os.path.join(CKPT_DIR, "jazz_lstm.pt")


def load_data(seq_len):
    """Load parquet + vocab, map chord strings to (root, quality) ids,
    bundle per-solo numpy arrays for fast random-chunk sampling."""
    ...
    chord_to_root, chord_to_qual, n_qualities = build_chord_tables(chord_vocab)
    # Defaults for any chord string that somehow isn't in the vocab table
    ...
    solos = []
    for melid, g in df.groupby("melid", sort=False):
        if len(g) <= seq_len + 1:
            continue
        solos.append({
...
def sample_batch(solos, batch_size, seq_len, rng):
    """Random fixed-length chunks across the corpus.

    At position j in the chunk (0..seq_len-1):
        prev_token = tokens[start + j]            # what was played just before
        chord/bar  = features at tick start+j+1   # the tick we are predicting
...
def main():
    ...
    print("loading data...")
    solos, meta, n_qualities = load_data(args.seq_len)
    n_ticks = sum(len(s["token"]) for s in solos)
    print(f"  solos usable        : {len(solos)}")
    print(f"  ticks total         : {n_ticks:,}")
    ...
    # Uniform-class baseline loss for reference: log(vocab_size)
    ...
    for ep in range(args.epochs):
        ...
        for step in range(args.steps_per_epoch):
            pt, cr, cq, bp, tg = sample_batch(solos, args.batch_size, args.seq_len, rng)
            pt, cr, cq, bp, tg = pt.to(device), cr.to(device), cq.to(device), bp.to(device), tg.to(device)
        ...
        print(f"epoch {ep+1:>3d}/{args.epochs}  loss={avg:.4f}  "
              f"lr={optim.param_groups[0]['lr']:.5f}  elapsed={elapsed:.0f}s")

...
class MusicPredicitonModel(nn.Module):
    def __init__(self,num_chord, dur_embed = 10, hidden_layers = 512, lstm_layers = 2, pitch_classes=12, chord_embed = 32, pitch_class_embed =16, dropout = 0.3):
        super().__init__()
        ...
        self.lstm = nn.LSTM(self.input_dim, hidden_layers, lstm_layers,dropout=dropout, batch_first = True)
...
def build_batch(data, batch_size, seq_length, rng):
    bp_ , bd_ , bc_, tp_, td_, bb_ = [], [], [], [] ,[], [] # empty batches
    indices = rng.permutation(len(data))[:batch_size]

    for i in indices:
        x = data[i]
        j = len(x["target_pitch_class"])
        start = 0 if j <= seq_length else int(rng.integers(j-seq_length))
        length = min(j, seq_length)
        end = start + length
        def pad(a, fill=0):
    ...
    return( torch.from_numpy(np.stack(bp_)).to(device),
        torch.from_numpy(np.stack(bd_)).to(device),
        torch.from_numpy(np.stack(bc_)).to(device),
        torch.from_numpy(np.stack(bb_)).to(device),
        torch.from_numpy(np.stack(tp_)).to(device),
        torch.from_numpy(np.stack(td_)).to(device), )
...
def train(model, prepped, batch_size,prepped_val, seq_length=64, epochs=3000, lr=0.001, ):
    ...
    for epoch in range(epochs):
        ...
        prev_pc, prev_dur, cur_chord, beat_pos, target_pitch_class, target_dur = build_batch(prepped, batch_size, seq_length, rng)
        total_loss = 0
        # bp_ , bd_ , bc_, tp_, td_, bb_ = [], [], [], [] ,[], [] # empty batches
        ...
        if epoch % 100 == 0:
            ...
            with torch.no_grad():
                ...
                val_prev_pc, val_prev_dur, val_cur_chord, val_beat_pos, val_target_pitch_class, val_target_dur = build_batch(prepped_val, batch_size, seq_length, rng)
...
trained_model = train(model,prepped=train_data, batch_size=24, epochs= 3000, prepped_val=val_data)
